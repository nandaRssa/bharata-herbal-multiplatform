        ...
        <div class="flex items-center justify-between mt-3 mb-2">
            <div class="flex items-center gap-1">
                <div class="flex text-amber-400">
                    @for ($i = 1; $i <= 5; $i++)
                        <svg class="w-3.5 h-3.5 {{ $i <= round($product->rating) ? 'fill-current' : 'text-gray-200' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <span class="text-[11px] font-bold text-gray-700 ml-0.5">{{ number_format($product->rating, 1) }}</span>
                <span class="text-[10px] text-gray-400">({{ $product->rating_count }})</span>
            </div>
            <div class="bg-blue-50 text-blue-700 text-[10px] font-bold px-2 py-0.5 rounded-md border border-blue-100 flex items-center gap-1">
